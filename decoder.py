...
class DecoderLayer(nn.Module):
    def __init__(self, d_model, d_ff,drop_prob):
        super().__init__()
        self.self_attn = MultiHeadAttention(d_model,dropout=drop_prob)
        self.cross_attn = MultiHeadAttention(d_model, dropout=drop_prob)
...
class Decoder(nn.Module):
    def __init__(self, dec_voc_size, max_len, d_model, d_ff, n_layers, pad_id,drop_prob, device='cuda'):
        super().__init__()
        self.embedding = TransformerEmbedding(dec_voc_size, d_model, max_len, pad_id, drop_prob,device)
        self.layers = nn.ModuleList([
            DecoderLayer(d_model, d_ff,drop_prob)
            for _ in range(n_layers)
        ])
        self.fc = nn.Linear(d_model, dec_voc_size)
        #self.history = []
